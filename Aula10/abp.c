/*******************************************************************************
 ...
 Autor : Daniela Pereira Simões    Data : Maio de 2016
...
PtABPNode ABPMaxRep (PtABPNode proot)
{
    ...
    PtABPNode search = proot;

    while(search->PtRight != NULL){
        search = search->PtRight;
    }
...
unsigned int ABPEvenCount (PtABPNode proot) {
    ...
    if ((q = QueueCreate(sizeof(PtABPNode))) == NULL) {
        Error = NO_MEM;
        QueueDestroy(&q);
        return 0;
    }
    ...
    while(!QueueIsEmpty(q)){
        QueueDequeue(q, &node);
        if(ABPElement(node) % 2 == 0) Even++;

        if(node->PtLeft != NULL){
            QueueEnqueue(q, &node->PtLeft);
        }

        if(node->PtRight != NULL){
            QueueEnqueue(q, &node->PtRight);
        }
    }
...
int ABPMultSum (PtABPNode proot, int pvalue)
{
    ...
    if((s = StackCreate(sizeof(PtABPNode)))== NULL){
        Error = NO_MEM;
        StackDestroy(&s);
        return 0;
    }
    ...
    while(!StackIsEmpty(s)){
        if(node != NULL){
            ...
            if(!StackIsEmpty(s)){
                StackPop(s, &node);
                if (ABPElement(node) % pvalue == 0) sum+=ABPElement(node);
                node=node->PtRight;
                StackPush(s, &node);
            }
        }
...
int ABPTotalSum (PtABPNode proot)
{
    ...
    var = proot->Elem + ABPTotalSum(proot->PtLeft) + ABPTotalSum(proot->PtRight);
...
unsigned int ABPOddCount (PtABPNode proot)
{
    ...
    if(proot != NULL){
        if(ABPElement(proot)%2 != 0){
            var = ABPOddCount(proot->PtLeft) + ABPOddCount(proot->PtRight) + 1;
        }else{
            return ABPOddCount(proot->PtLeft) + ABPOddCount(proot->PtRight);
        }
        ABPOddCount(proot->PtRight);
    }
